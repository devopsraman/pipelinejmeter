		...
		sh ' /usr/local/bin/jmeter -n -t MVP1.0MaxLTV.v2.jmx -l /usr/local/bin/jmeter.jtl  ' 
		   ...
		   stage ('build jmeter container') {
			  steps {
				 ...
				 sh' /tmp/docker run -dit -v /tmp:/tmp -p 1000:1000 --name master lazzurs/jmeter /bin/bash '
				 sh '/tmp/docker exec -it master /bin/bash -c "jmeter -n -t /tmp/MVP1.0MaxLTV.v2.jmx -l /tmp/jmeter.jtl"'
